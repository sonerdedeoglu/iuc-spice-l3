#!/usr/bin/env python3
"""Finalize the LST.009 template candidate.

Actions:
- Move the currently active LST.009 template page under `Arşiv - Kaldırılan Şablonlar`
  and rename it with `KALDIRILDI - ...`.
- Promote the draft LST.009 template candidate to the active template path/title.
...
def main() -> None:
    if not OLD_DIR.exists():
        raise FileNotFoundError(f"Current active LST.009 template page not found: {OLD_DIR}")
    if not DRAFT_DIR.exists():
        raise FileNotFoundError(f"Draft LST.009 template candidate page not found: {DRAFT_DIR}")
    ...
    print("[DONE] LST.009 template candidate promoted to active template.")
